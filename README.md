...
- **不使用 `--force`**。若本機 `output/` 的歷史與 `origin/gh-pages` 分歧，推送會直接失敗，
  需要人工判斷後處理，不會靜默覆蓋遠端。
- deploy 會將 world-map/catalog.json、versioned `world-map/snapshots/<region>/<version>/...` 與 world-map/images/* 納入 metadata/image commit；未版本化的 legacy world-map alias 已不再發布；
